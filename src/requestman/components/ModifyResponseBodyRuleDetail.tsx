...
function validateDynamicScript(code: string): string | null {
  if (!hasModifyResponseBodyFunction(code)) return '需定义 modifyResponse(args) 方法';
  return null;
}
      ...
      return <Collapse
        ...
        items={[{
          ...
          children: <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Form.Item
              label={c.responseBodyMode === 'dynamic' ? 'JavaScript 代码' : '替换后的响应体'}
              validateStatus={dynamicScriptError ? 'error' : ''}
              help={dynamicScriptError ?? (c.responseBodyMode === 'dynamic' ? '需定义 modifyResponse(args) 并返回最终响应体' : '命中后会直接替换原始响应 body')}
              layout="vertical"
