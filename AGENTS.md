...
- macOS UI must be implemented with AppKit; SwiftUI is prohibited, including SwiftUI content embedded through `NSHostingView` or `NSHostingController`. Use Apple-provided native controls and system styles. Do not recreate controls or add glass/material wrappers merely to achieve a glass appearance. Follow the [native control constraints](apps/macos/AGENTS.md#原生控件约束) for all macOS UI work.
