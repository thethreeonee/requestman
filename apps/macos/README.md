...
项目和请求修改行统一为 40 pt，请求修改仅显示名称。点击项目整行可展开或收起；项目名称通过右键菜单重命名。项目菜单提供整组启用/禁用、复制、重命名、图标选择、导出整组及删除；请求修改菜单提供启用/禁用、重命名、复制、导出及删除。禁用项目会停止匹配该项目中的所有请求修改，并保留每条请求自身的启用状态；重新启用项目后恢复原有选择。
