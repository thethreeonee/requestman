...
更多菜单位于展开侧栏的原生工具栏中、收起按钮左侧，侧栏收起时隐藏。菜单采用 `NSMenuToolbarItem` 的系统按钮外观，依次提供“复制完整 URL / 复制原始请求为 cURL / 复制修改后请求为 cURL”；两种 cURL 分别取修改前后的方法、URL、Header 和 Body。URL、Header 或正文记录不完整时禁用相应复制项并提供原因；已脱敏的请求头不会恢复凭证，导出命令包含脱敏说明。
...
完整 gzip / deflate 内容仅在查看时后台解压，上限 256 KiB；JSON 上限为 256 KiB、64 层、每侧 4,096 节点。超限、未知编码、二进制或不合法 JSON 通过源码/十六进制查看并注明原因；不把未采集、断流或截断当作空 Body。URL 和 Header 的已有记录上限仍生效，截断与脱敏字段不参与不可靠的差异判断。具体设计和验收见 [请求详情设计](Docs/Design/request-inspector.md)。

字段值为字符串时，悬停会后台尝试解析为 JSON。解析成功后，复制按钮旁显示“查看 JSON”图标；点击用系统 Popover 和相同字段树查看，支持继续展开、复制及检查嵌套的 JSON 字符串。原字段类型和复制内容不变；普通文本、超限内容以及截断或脱敏的 Header 不显示预览图标。
