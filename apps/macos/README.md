...
| 请求流程 | 设置/移除 Header、修改方法、改写请求 URL、替换文本 Body、Mock、返回重定向 |
| 响应流程 | 设置/移除 Header、状态码、替换文本 Body、返回重定向；Mock 同样经过响应流程 |
...
文件夹与请求修改行统一为 30 pt，子项仅显示名称，名称左边缘与文件夹名称对齐。单击选择，双击文件夹整行或单击原生箭头展开/收起；左右键保留原生展开行为，子项位移/淡入淡出和箭头旋转为 180 ms，减少动态效果时即时切换。保留当前流程和会话内展开状态。搜索与添加保持在底部。数量右对齐，更多按钮占用固定位置，悬停或键盘选中时显示；右键菜单保持可用。未选中行悬停显示浅灰背景，以 Core Animation 在移入 120 ms、移出 160 ms 内淡入淡出，减少动态效果时即时切换。系统选中背景优先。“修改图标”提供六列、48 个候选的原生图标网格，仅显示预览，名称通过悬停提示和辅助功能提供。项目菜单提供添加请求修改、整组启用/禁用、复制、重命名、图标选择、导出整组及删除；请求修改菜单提供启用/禁用、重命名、复制、导出及删除。禁用项目会停止匹配该项目中的所有请求修改，并保留每条请求自身的启用状态；重新启用项目后恢复原有选择。
