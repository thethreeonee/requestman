...
摘要顶部直接以 URL 为标题，单行保留首尾、中间省略，悬停显示已记录的完整地址。点击 URL 打开原生 Popover，地址可选择、折行，超长时可滚动，并提供“复制完整 URL”按钮；记录已截断时明确提示地址不完整并禁用完整复制。摘要保留方法、状态码和耗时；URL 下方的“查询参数”入口单独展示原始/最终查询项，不混入请求体。命中规则时显示入口，可查看项目、环境和执行步骤；未命中时隐藏此行。

“修改前 / 修改后 / 修改对比”显示模式位于展开侧栏顶部标题工具栏，紧邻更多按钮左侧。原生 `NSSegmentedControl` 使用 `.automatic` 分段样式与 `.large` 尺寸，macOS 26+ 使用控件自身的 `.capsule` 形状，macOS 27+ 设置 `.tabs` 语义。整个 Inspector 共用一份显示模式，默认“修改后”，在四个内容 Tab、侧栏收起重开及选择不同记录时保持。
