...
独立的原生 macOS Web 调试工具。以项目组织请求修改，每条修改包含请求、响应两条流程；标题栏通过“请求修改 / 请求日志”切换全局页面，环境管理收纳在设置中。设置顶部的“通用 / 连接 / 环境管理”与主界面的“请求修改 / 请求日志”均使用原生 `NSSegmentedControl`。界面依据 [设计稿](Docs/Design/README.md) 实现，浏览器扩展仍独立运行与发布。
...
上游配置在设置 →“连接”中，不出现在主工作区。配置了上游时不会失败后静默改走其他出口；不使用 HTTP 上游时仅沿用系统路由，仍可能经过其他网络软件的增强模式。上游认证暂未实现。监听期间也可编辑本地端口和上游配置，停止输入约 350 ms 后自动应用，无需手动启停。上游变更用于新连接，现有请求与 CONNECT 隧道继续使用原配置；端口变更自动切换监听并关闭现有连接，全局模式同步系统代理，浏览器模式不改系统代理并重新打开当前会话浏览器的新端口调试窗口。旧浏览器窗口仍固定使用旧端口，不自动关闭。无效配置不打断监听；新端口启动失败会尝试恢复原配置与原模式，设置页显示应用错误。
...
设置 →“连接”的 HTTPS 行提供“设置证书…”。点击后自动检查已有证书，依次生成本机 CA、安装到默认钥匙串并请求当前用户的 SSL 信任；仅需响应 macOS 授权提示。取消会保留已完成步骤，可再次点击继续；重复操作复用同一证书，不重复生成。引导可查看证书名称、有效期与 SHA-256 指纹，失败时提供重试和“钥匙串访问”入口。完成后 HTTPS 行显示“已完成配置”；重新打开连接页或返回应用时会只读检查证书状态，失效后恢复设置入口。证书就绪后新 CONNECT 连接自动解密，无需重新启停监听。已有透传隧道不能就地解密，需刷新页面或重新打开调试浏览器建立新连接；旧日志不会补全成明文。
...
授权只在用户主动配置证书时发生，配置阶段为当前应用建立持久的 CA 签名权限，新权限描述为“Requestman HTTPS 调试 CA”。已有证书复用原私钥与信任，修复签名权限不重新生成 CA。完成前会在禁止弹窗的条件下复核签名能力和实际 SSL 信任；启动、返回前台、状态检查及 HTTPS 解密均禁止钥匙串授权弹窗。钥匙串锁定或应用签名变化导致授权不可用时，在连接设置中提示点击“设置证书…”重新授权，不自动重弹。
