...
整个窗口及详情均取消底部状态栏，列表和详情内容延伸到底部；“仅显示变更”位于详情上方的内容说明行。侧栏使用系统背景，字段树和原始数据视图不再铺设独立白底。此轮原生窗口布局和外观仍待真实 App 运行验收。
...
内容只有一排“请求头 / 请求体 / 响应头 / 响应体”原生 `NSSegmentedControl`，macOS 26+ 通过控件自身的 `borderShape = .capsule` 使用原生胶囊外观，旧系统保留默认外观，不额外包装玻璃背景；分别保留搜索、滚动和展开状态。“原始 / 最终 / 差异”仍通过独立菜单选择。请求体、响应体的 JSON 树提供“原始数据”按钮，用只读 `NSTextView` 查看保留原有缩进、换行和字段顺序的文本；按钮随之变为“树形视图”，可切回字段树。此按钮只切换显示方式，不改变当前版本；非 JSON 内容直接显示文本或十六进制，不提供无效切换。原始响应指服务器返回，最终响应指代理发给客户端；Mock 无服务器原始响应，不伪造空对象。Header 和 JSON 字段只用背景区分增删改，不增加“已修改”标签。鼠标悬停行右侧的复制按钮以 0.15 秒淡入淡出（减少动态效果时直接切换），字段仅复制其值，容器复制整个 JSON 子树（搜索后仍复制完整子树）；也支持右键和 ⌘C。
