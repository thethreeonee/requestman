...
完整 Body 处理保留执行等待时限，不设置内容尺寸或共享载荷预算；2026-09-26 已按产品要求移除 URL/Header/Body 截断、凭据脱敏和应用层 JSON 解析上限，详见 [当前限制检查](LimitsAudit.md)。SSE 等持续响应不应默认等待完整 Body。响应头部发出后，不能再承诺修改状态码或执行整体响应替换；首版不提供任意流式脚本变换。
