...
| 凭据 | Authorization、Proxy-Authorization、Cookie、Set-Cookie、X-API-Key 和其他 Header 均保留实际值；详情比较和 cURL 导出不再使用脱敏占位符 | [RequestCURL.swift](../Requestman/Features/Requests/RequestCURL.swift) |
...
| 显示元数据 | 项目/工作流/环境/规则名 128 字符；错误 512 字符；步骤摘要 64 条、每条 128 字符。均不用于截断实际 URL、Header 或 Body |
...
详情和 cURL 脚本验证修改前后完整复制、凭据值比较，宿主通过 Swift 6 类型检查；未构建或启动 App，未进行真实浏览器流量验收。
